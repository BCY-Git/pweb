...
const stepsFor = (detail: Detail): number => DETAIL_STEPS[detail] ?? DETAIL_STEPS.medium
...
void main() {
  ...
  vec4 sum = texture(tMap, uv) * 0.2026;
  sum += (texture(tMap, uv + texel * st) + texture(tMap, uv - texel * st)) * 0.179;
  sum += (texture(tMap, uv + texel * (st * 2.0)) + texture(tMap, uv - texel * (st * 2.0))) * 0.124;
  sum += (texture(tMap, uv + texel * (st * 3.0)) + texture(tMap, uv - texel * (st * 3.0))) * 0.0672;
  sum += (texture(tMap, uv + texel * (st * 4.0)) + texture(tMap, uv - texel * (st * 4.0))) * 0.0285;
  vec4 col = sum;
  if (uGrain > 0.5) {
    float gv = (fract(sin(dot(gl_FragCoord.xy, vec2(12.9898, 78.233)) + iTime) * 43758.5453) - 0.5) * uGrainIntensity;
...
/**
 * React Bits 的 AcidSquares 组件 —— raymarching 有机辉光背景。
 *
 * 比 SlicedWaves 重（步进循环 + 可选双 pass 模糊），适合作整板块背景。
 * 内置性能优化：低分辨率渲染(dpr=0.5) + 30fps 降帧 + 视口外/切页暂停。
 */
const AcidSquares = ({
  color1 = '#5227FF',
  color2 = '#A855F7',
  ...
  useEffect(() => {
    ...
    // 模糊 pass 的乒乓 RenderTarget，按 drawingBuffer 尺寸重建
    let rtA: RenderTarget | null = null
    let rtB: RenderTarget | null = null
    const ensureTargets = () => {
      const bw = gl.drawingBufferWidth
      const bh = gl.drawingBufferHeight
      if (rtA && rtB && rtA.width === bw && rtA.height === bh) return
    ...
    const loop = (t: number) => {
      ...
      if (enableMouseRef.current) {
        ...
        m[0] = cur[0]
        m[1] = cur[1]
        mouseActive.current += 0.05 * (mouseActiveTarget.current - mouseActive.current)
    ...
    // WebGL 上下文只建一次，props 更新走下方 ctxMap 通道
