...
/**
 * React Bits 的 MoltenMetal 组件 —— 液态金属熔融背景。
 * 基于 WebGL（ogl）实现 caustic 焦散纹理，带鼠标交互。
 *
 * 用作 Hero 首屏的全屏背景，配合半透明遮罩让前景文字清晰可读。
 */
const MoltenMetal = ({
  color1 = '#5227FF',
  color2 = '#FF9FFC',
  ...
  useEffect(() => {
    ...
    // 【性能】强制低分辨率渲染（dpr=0.5），液态金属是模糊流体效果，
    // 降低分辨率几乎无视觉损失，但 shader 计算量降到原来的 ~1/9。
    // CSS 会把 canvas 拉伸铺满容器。
    ...
    // 【性能】降帧到 ~30fps：液态金属流动缓慢，30fps 足够流畅，GPU 负载减半
    let lastFrame = 0
